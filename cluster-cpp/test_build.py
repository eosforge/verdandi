...
class BuildEntryTests(unittest.TestCase):
    ...
    def temporary_project(self):
        ...
        directory = tempfile.TemporaryDirectory(prefix="cluster-cpp-build-test-", dir=temporary_root)
    ...
    def test_environment_is_child_only_and_avoids_empty_search_paths(self):
        ...
        inherited = {"PATH": "parent-bin", "LD_LIBRARY_PATH": "", "CMAKE_BUILD_PARALLEL_LEVEL": "8"}
        with patch.object(build, "ROOT", project), patch.dict(os.environ, inherited, clear=True):
            actual = build.environment(project / "deps")
            self.assertEqual(dict(os.environ), inherited)
        self.assertEqual(actual["LD_LIBRARY_PATH"], str(project / "build/tools/gcc-16.2.0/lib64"))
    ...
    def test_explicit_parent_runtime_path_is_preserved(self):
        project = self.temporary_project()
        with patch.object(build, "ROOT", project), patch.dict(os.environ, {"LD_LIBRARY_PATH": "parent-lib"}, clear=True):
            actual = build.environment(project / "deps")
    ...
    def run_fixture(self, *, no_tests=False):
        ...
        project = self.temporary_project()
        source = project / "cluster-cpp"
        output = project / "build/cluster-cpp/core-debug"
        source.mkdir()
        ...
        def execute(command, env):
            ...
            if os.name == "nt" and command[0] == "ctest":
                command += ["-C", "Debug"]
            return subprocess.run(command, cwd=project, env=env, check=True, capture_output=True, text=True, encoding="utf-8")

        execute(["cmake", "-S", source, "-B", output, "-DBUILD_TESTING=OFF", "-DFIXTURE_NO_TESTS=" + ("ON" if no_tests else "OFF")], dict(os.environ))
    ...
    def test_cached_disabled_tests_are_reenabled_and_executed(self):
        ...
        self.assertIn("BUILD_TESTING:BOOL=ON", (output / "CMakeCache.txt").read_text(encoding="utf-8"))
    ...
    def test_empty_suite_fails_the_entry(self):
        ...
        self.assertIn("No tests were found", error.exception.stderr + error.exception.stdout)
