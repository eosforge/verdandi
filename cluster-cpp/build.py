...
SOURCE = ROOT / "cluster-cpp"
PREFIX = ROOT / "build/deps/cluster-cpp/linux-gcc16/install"
...
def generated(check, env):
    ...
    for schemas, names, destination in (
        ...
        with tempfile.TemporaryDirectory(prefix="cluster-cpp-proto-", dir=ROOT / "build/tmp") as temporary:
            run(
                [
                    protoc,
...
def main():
    ...
    if sys.platform != "linux":
        ...
        raise SystemExit(2)
    prefix = ROOT / "build/deps/cluster-cpp/linux-gcc16-tsan/install" if options.profile == "tsan" and not options.core_only else PREFIX
    ...
    if options.command in {"generate", "check-generated"}:
        ...
        return
    if options.profile == "tsan" and not options.core_only and not (ROOT / "build/deps/cluster-cpp/artifacts-tsan.json").is_file():
    ...
    configure = [
        ...
        *[
        ...
        "-DVERDANDI_CONTRACT_SEMANTIC=" + options.contracts,
        "-DPython3_EXECUTABLE=" + sys.executable,
        "-DVERDANDI_MEASURE_ALLOCATIONS=" + ("ON" if options.measure_allocations else "OFF"),
        "-DVERDANDI_BUILD_BENCHMARKS=" + ("ON" if options.benchmarks else "OFF"),
    ]
    run(configure, env)
