...
void vectors(const Identity& identity) {
    auto json = test::read(std::filesystem::path(VERDANDI_FIXTURES) / "admission-v5.json");
...
int main() {
    try {
        ...
        auto identity = Identity::load(std::filesystem::path(VERDANDI_FIXTURES) / "star-a", endpoint);
        CHECK(identity);
        CHECK(!Identity::load(std::filesystem::path(VERDANDI_FIXTURES) / "expired", endpoint));
        CHECK(!Identity::load(std::filesystem::path(VERDANDI_FIXTURES) / "star-a", *Endpoint::parse("192.0.2.1:7443")));
        vectors(**identity);
        signatures(**identity);
        std::cout << "PASS shared v5 vectors, raw-byte Ed25519, local certificate validation\n";
        return 0;
