...
struct Stream {
    ...
    bool finished{};
    explicit Stream(wire::StarTransport::Stub& stub) {
...
wire::Hello admit(const Identity& identity, const std::string& supervisor, const std::string& advertise) {
    ...
    request.set_role(wire::ROLE_STAR);
...
int main(int argc, char** argv) {
    try {
        ...
        auto stub = wire::StarTransport::NewStub(channel);
        {
        ...
        CHECK(first.ping(43));
        {
            ...
            auto isolated_stub = wire::StarTransport::NewStub(isolated);
