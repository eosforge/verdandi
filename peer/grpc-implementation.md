...
后续 C++26 迁移已按 [设计稿](cpp26-skeleton-design.md) 在独立 [peer-cpp/](../peer-cpp/README.md) 实施和验证, 保留本文的 Rust 实现作对照.
...
| Star / Planet | Rust Tonic `PeerTransport.OpenSession`, 每条连接一个双向消息流 |
...
生产 Peer 已移除自定义 FrameReader/FrameWriter. 旧 MessageID 清单继续作为历史保留,
...
| Peer 登记 | 本地总期限覆盖连接、Challenge 和 Register; 失败重试保留首次 CAS 基线 |
| 心跳 | 单方向最多一个待匹配编号; 绝对期限覆盖入队、实际传输和匹配 Pong |
| Peer 时间配置 | 网络时间统一为 1 ms..24 h, 退避最小值不得大于最大值; 创建任务前拒绝极端输入 |
| Peer 并发配置 | 入站连接 1..65536, 并发拨号 1..4096, 诊断事件队列 1..65536; 配置上界不是容量认证 |
