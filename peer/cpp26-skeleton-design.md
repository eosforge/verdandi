...
日期: 2026-09-11. 状态: 独立 peer-cpp/ 连接回归及 TSan 已通过; 其余设计验收按维护者要求暂停.
...
| 网络协议 | 保持 v4, `Admission.Challenge/Register` 与 `PeerTransport.OpenSession` |
| 目录职责 | 在 `peer-cpp/common`, `peer-cpp/star`, `peer-cpp/planet` 保持责任划分 |
| 产物 | Star 为 `peer`, Planet 为 `planet`, 第一阶段不产出 Windows 二进制 |
...
`PeerId`, `Principal`, `SessionGeneration` 与 `MemberEpoch` 分离, 防止整数和字符串混用.
...
以下为首版实际源码布局. 验证是否完成以 `peer-cpp/validation.md` 的证据为准.
...
peer-cpp/
    ...
    common/
        include/verdandi/peer/     # 两个角色实际共用的少量声明
        src/
            config.cpp / options.hpp
            identity.cpp / admission.cpp
            grpc_session.cpp
            runtime.cpp           # 生命周期与有限结构化日志
...
`peer_star` / `peer_planet` 只依赖 `peer_core`. 入口再组合角色与 `peer_runtime`, 后者依赖 `peer_wire` / gRPC / crypto.
公开测试凭据复用 `peer/tests/fixtures`, 进程故障循环复用 `testkit/services.py`, 不复制两份状态机和夹具.
...
维护者随后允许新建目录, 实施采用独立 `peer-cpp/`, 当前 Rust 仍位于 `peer/`, 两者使用独立构建输出.
...
`max_peers` 每角色分别计数, Star 包含自身, Planet 不占 Star mesh 名额, 离线记录不自动删除.
...
| gRPC C++ / grpc_cpp_plugin | 1.84.0, 插件使用同一份源码 | `build/deps/peer-cpp`, 生成工具放 `build/tools` |
...
首版依赖产物放 `build/deps/peer-cpp/linux-gcc16/install`, 版本由来源锁固定,
对象与二进制放 `build/peer-cpp/<profile>`, core-only 使用独立的 `core-<profile>`.
...
`peer-cpp/common/src/generated`; 普通编译不运行 protoc. 保留字段编号和 reserved 项, 不复活 MessageID 分派.
校验命令在项目临时目录重新生成后逐字节比较, 不在检查阶段修改源码.
锁定 protoc、插件及参数, 去掉时间/机器路径等不稳定生成输入.

`proto/peer_transport.proto` 的 Supervisor 旧帧协议注释已修正, 并明确逻辑会话与物理连接的区别;
...
独立 `peer-cpp/` 已实现连接骨架, 获准的依赖已在 Ubuntu 项目内构建, Debug/Release、ASan/UBSan、
跨语言互通和修复后一小时故障循环已有实测证据. 完整 TSan 已通过, 保存结果后按维护者要求停止执行.
编译期负例、契约关闭对照、规模/分配成本及独立推流对照尚未完整验收; TSan 通过不自动代表 M0..M4 全部完成.
实际证据和缺口以 [C++ 验证记录](../peer-cpp/validation.md) 为准, 不把设计文本当作已实现或已通过的证明.
