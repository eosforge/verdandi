...
Result<Member> decode_member(const wire::RegistrationResponse::Member& value) {
    auto id = PeerId::parse(value.peer_id());
    auto principal = Principal::parse(value.principal());
    auto address = Endpoint::parse(value.advertise());
    if (!id || !principal || !address || address->text() != value.advertise() ||
        (value.role() != wire::NODE_ROLE_STAR && value.role() != wire::NODE_ROLE_PLANET)) {
        return std::unexpected(Error{ErrorCode::identity, "Invalid encoded member"});
    }
    Member result{value.cluster_id(), *id, *principal, *address, MemberEpoch{value.epoch()}, value.role() == wire::NODE_ROLE_STAR ? Role::star : Role::planet,
                  value.group()};
...
Result<PeerId> Identity::new_id() {
    PeerId id;
    if (RAND_bytes(id.bytes.data(), id.bytes.size()) != 1) {
        return std::unexpected(Error{ErrorCode::identity, "System randomness unavailable"});
    }
    id.bytes[6] = static_cast<std::uint8_t>((id.bytes[6] & 15) | 0x40);
    id.bytes[8] = static_cast<std::uint8_t>((id.bytes[8] & 63) | 0x80);
    return id;
}
...
Result<Member> Identity::verify(std::string_view payload, std::string_view signature) const {
    ...
    constexpr char domain[] = "verdandi-admission-v4";
