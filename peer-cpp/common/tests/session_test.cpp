...
#define CHECK(condition)                                                                                                                                       \
    do {                                                                                                                                                       \
        if (!(condition))                                                                                                                                      \
            throw std::runtime_error("Session check failed at line " + std::to_string(__LINE__));                                                              \
    } while (false)
...
struct RecordingPolicy final : Policy {
    ...
    Result<std::vector<SessionGeneration>> accept(const Member&, Direction, SessionGeneration, const std::optional<DialTarget>&) override {
        ++installations;
        return std::vector<SessionGeneration>{};
    }
    void closed(SessionGeneration, std::optional<ErrorCode>, Clock::time_point) override {}
    std::vector<DialTarget> due(Clock::time_point, std::size_t) override {
        return {};
    }
    void failed(const DialTarget&, ErrorCode, Clock::time_point) override {}
