...
CACHE = PROJECT / "build/deps/peer-cpp"
LOCK = json.loads((PROJECT / "peer-cpp/dependencies.lock.json").read_text())
...
def build(profile):
    ...
    sources = {item["name"]: source(item) for item in LOCK["sources"]}
    env = dict(os.environ)
    env["PATH"] = str(GCC / "bin") + os.pathsep + env.get("PATH", "")
    env["LD_LIBRARY_PATH"] = str(GCC / "lib64") + os.pathsep + env.get("LD_LIBRARY_PATH", "")
