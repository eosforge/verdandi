...
namespace astra {
class PulsarAuthority {
public:
    // 加载既有 Supervisor 格式的账号和密钥, 验证两个监听端点的证书授权; 失败不返回部分对象.
    static Result<std::unique_ptr<PulsarAuthority>> load(const std::filesystem::path& directory, const Endpoint& admission, const Endpoint& pulse);
