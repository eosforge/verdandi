...
新增证据和性能限制见 [补充报告](qualification-20260912.md).
随后完成模块与测试入口整理, 四种配置、短故障循环和安装检查结果见 [维护性整理记录](maintenance-20260912.md).

机器可读的场景、产物摘要、资源采样和历史失败索引见
[汇总结果](../testkit/results/cluster-cpp-foundation-20260911.json). 该历史结果的状态 regression_passed_work_paused 保留, 不回写后续结果.
...
最终接收/失败状态复查后的日志为 `build/cluster-cpp/debug-accepted.log`, `asan-accepted.log` 和 `release-hour-fixed.log`.
...
最终 TSan 日志为 `build/cluster-cpp/tsan-final.log`, CTest 用时 16.76 秒;
RPC 报告为 `cluster-cpp-rpc-1789132888205834959.json`, 进程报告为 `services-1789132925763255264.json`.
...
以下为修复后一小时入口的 Release 前置场景, 来自 `cluster-cpp-rpc-1789125171274001193.json`:
...
代码和测试集中在 `cluster-cpp/`. Rust 保留在 `cluster/`, Supervisor 保持 Go.
Redis SDK、数据存储/复制、Catalog/Registry 转发与 Admin 不属于本轮迁移范围.

## 设计验收项与测试对应

本轮 TSan 已通过, 但它不是整个设计文档的最后一项验收.
连接功能已实现并取得下列证据. 以下为 2026-09-11 暂停时的缺口, 2026-09-12 进展另见补充报告:
...
2026-09-11 的规模为最多 4 个 Star 和 2 个 Planet; 不能由此声称容量参数允许的全部规模已经得到验证.
...
- Bearer 预先发送问题在原 Rust Common 的握手函数中同样存在, 已修改公共路径, Star/Planet 共用修复.
  Go Supervisor 的 Challenge/Register 先进行账号鉴权, SDK 尚无该 StarTransport 接口, 不受这条握手路径影响.
...
- 新 C++ 控制队列的期限次序问题不在 Rust 同一位置扩散: Rust 按每次写入的绝对期限等待有界通道,
  没有“只检查队首期限”的数组算法. Go Supervisor 的准入是 unary RPC, 现有 SDK 不维护 Star Hello/Ping 队列.
