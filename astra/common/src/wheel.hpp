...
public:
    ...
    static constexpr std::uint64_t limit = std::numeric_limits<std::uint64_t>::max() >> (64 - Levels * Bits);
    ...
    [[nodiscard]] bool schedule(Node& node, std::uint64_t delay) noexcept {
        ...
        cancel(node);
        node.deadline_ = clock_ + std::max(delay, std::uint64_t{1});
        insert(node);
        return true;
    }
    ...
    void tick(F&& fire) {
        ...
        for (auto level = std::min(Levels - 1, static_cast<std::size_t>(std::countr_zero(clock_)) / Bits); level > 0; --level) {
            // bucket 接管整个高层桶. 期间无回调且不抛异常, 无需逐节点维护旧链表的反向链接.
            // 摘桶为 O(1), 后续重新分层仍需遍历桶内全部节点.
            auto* bucket = std::exchange(wheels_[level][(clock_ >> (level * Bits)) & mask], nullptr);
        ...
        ready_ = std::exchange(wheels_[0][clock_ & mask], nullptr);
        if (ready_ != nullptr) {
...
private:
    ...
    void insert(Node& node) noexcept {
        ...
        auto& head = wheels_[level][(node.deadline_ >> (level * Bits)) & mask];
        node.next_ = head;
        node.pprev_ = &head;
        if (head != nullptr) {
