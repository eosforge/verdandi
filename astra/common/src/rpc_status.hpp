...
// 由于部分简单的会话回收逻辑内部只需要基础的枚举码就能运行, 特在此分离本方法，从而避免单纯为取得底层 code 还要被迫发生字符串内存临时动态申请分配的 Status
// 包装对象开销。 参数 status: 传入的 grpc 底层状态枚举。
constexpr Status::Code rpc_error_code(grpc::StatusCode status) noexcept {
    switch (status) {
    case grpc::StatusCode::UNAUTHENTICATED:   // 未认证的情况
    case grpc::StatusCode::PERMISSION_DENIED: // 无权限，拒绝访问的情况
        // 以上身份不被认可，归为安全凭证与身份错误类别。
        return Status::Code::identity;
    ...
    case grpc::StatusCode::UNAVAILABLE: // 网络设备离线导致服务彻底不可用
        ...
        return Status::Code::transport;
    default:
        // 对于那些与框架协议紧密强相关以及所有剩余未做特殊明确归属划定的杂项故障代码，全部兜底退化认为是网络协议层面或者版本校验层面的不兼容错误。
        return Status::Code::protocol;
    }
}

// 封装状态码并结合固定错误诊断文本生成标准 Status 对象结构的帮助函数。
// 直接从发生失败状态的 grpc::Status 去快捷构造出一个持有完全独立可控本地写死固定诊断文本的标准 Status 实例，
// 在此严格规定必须只去提取其中的错误状态码核心信息，坚决抛弃不保留由于外部远端带入的任何附带的 message 或者其它危险的 details 动态错误明文描述内容。
// 参数 status: gRPC 核心对象，包含调用期间的全部状态。
inline Status rpc_error(const grpc::Status& status) {
