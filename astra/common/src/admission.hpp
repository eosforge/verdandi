...
private:
    // Admission::RegistrationCall 结构体: 封装了一次 gRPC Register 调用的所有相关数据和状态。
    ...
    // registration_: 当前在途的 Admission::RegistrationCall 实例指针，无在途时为 nullptr。
    std::shared_ptr<Admission::RegistrationCall> registration_;
};
