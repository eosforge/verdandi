...
std::string json_string(std::string_view value) {
    ...
    // hex 为 JSON 控制字节编码表, 按半字节索引, 不用于身份编码.
    constexpr char hex[] = "0123456789abcdef";
    // result 拥有带双引号的 JSON 字符串, 字节逐项转义后补末尾引号.
    std::string result = "\"";
    for (char byte : value) {
        // c 以无符号字节处理, 避免 char 符号扩展影响控制字符判断.
    ...
    // 在已有缓冲追加结束引号后返回, 避免左值 operator+ 再复制整段转义结果.
    result += '"';
    return result;
}
...
// status 方法实现
// - status (const Policy::State&): 包含了详细的网络运行指标的数据结构.
// - id (const Id&): 节点的唯一身份.
void Logger::status(const Policy::State& status, const Id& id) const {

    // 格式化上游活跃成员(如果有的话)
    const auto upstream = status.active_member ? std::format(R"({{"id":{},"group":"{}","address":"{}"}})", json_string(status.active_member->id), status.active_member->group, status.active_member->address.text()) : "null";

    // 格式化并写入网络状态 JSON.直接调用写方法, 并带上一系列经过校验和统计的值.
    write("status", std::format(R"({{"id":{},"initialized":{},"members":{},"inbound":{},"outbound":{},"planet_inbound":{},"candidates":{},"upstream":{}}})", json_string(id), status.initialized, status.members, status.inbound, status.outbound, status.planet_inbound, status.candidates, upstream));
}
