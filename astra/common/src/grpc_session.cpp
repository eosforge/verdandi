...
public:
    ...
    ClientSession(const Config& config, const Identity& identity, SessionGeneration generation, std::shared_ptr<const proto::astra::v1::Hello> hello,
        ...
        : RpcSession(config, Policy::Direction::outbound, generation, std::move(hello), target, std::move(wake)) {
...
RpcSession::RpcSession(const Config& config, Policy::Direction direction, SessionGeneration generation, std::shared_ptr<const proto::astra::v1::Hello> hello,
...
Result<std::vector<SessionGeneration>> RpcSession::receive(const proto::astra::v1::SessionPacket& packet, Policy& policy, const Identity& identity,
    ...
    if (packet.has_rejection()) {
        return std::unexpected(
            Status{packet.rejection().code() == proto::astra::v1::PROTOCOL_ERROR_CODE_UNAUTHORIZED ? Status::Code::identity : Status::Code::protocol,
...
void RpcSession::cancel(Status::Code error) {
    if (!error_ && !done()) {
        error_ = error;
...
Policy::Direction RpcSession::direction() const {
    return direction_;
}
...
std::optional<Status::Code> RpcSession::error() const {
...
AcceptedSession::AcceptedSession(grpc::CallbackServerContext* context, const Config& config, SessionGeneration generation,
    ...
    : RpcSession(config, Policy::Direction::inbound, generation, std::move(hello), std::nullopt, std::move(wake)), context_(context) {}
...
void AcceptedSession::finish_call(Status::Code error) {
    // 将上层会话模块计算出的内部错误转换为 gRPC 对外公开的标准状态码系列以发回对端。
    const auto status = error == Status::Code::identity                                      ? grpc::StatusCode::PERMISSION_DENIED
                        : error == Status::Code::protocol || error == Status::Code::conflict ? grpc::StatusCode::INVALID_ARGUMENT
