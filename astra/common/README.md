...
当前观察目录按 Scope 索引, 每次可见变化扫描该 Scope 的活动订阅, 仅向全范围或命中目标的订阅保存正文; 无关精确订阅仍推进连续覆盖位置并可能发送空增量帧. 尚未实现按精确 Key/UUID 分桶后只唤醒命中者的优化, 不能将它写成现有性能保证; 相关边界见 [审核](../../docs/review.md#performance). 纯续租或仅 order 变化不经过内容观察列表. 捕获基线与挂入观察使用同一提交边界, 空目标等待项有预算, 最后观察者退出后释放观察目录.
...
- 全量安装在锁外准备候选原生根和必要投影变更, 最终重查期间发生的版本/身份/期限变化. 原生根、投影根、水位和完整安装位置一起发布, 不能先 ACK 再补索引. 冲突导致的重新准备受恢复预算限制, 不无限占锁重建或覆盖其他来源的新状态.
...
完整来源组快照覆盖该来源、本域的全部 Scope, 空组也有完整边界. 分页可按 Scope 组织, 但不构成多个独立恢复位置; 新 Scope 和并发更新从组基线之后的连续历史衔接. 半份快照不改变已安装来源组或确认位置, 不以一个 Scope 完成当作组完成. 单条缺失按 [精确回补](../../proto/README.md#repair) 处理, 历史断档才恢复整个来源组.

来源组的原子恢复会比旧单 Scope 恢复持有更多准备状态, 这是已选组版本的代价. 组记录数、元数据、载荷、冻结页、投影准备及在途引用均有预算; 容量与失败分类遵循下方 [恢复预算](#capacity), 不靠分页声称总准备内存恒定. 失败保留旧完整状态, 不丢弃部分 Scope 后宣称成功.
