...
public:
    ...
    // 成功的话会深拷贝内部所需的特定值; 如果发现身份认证错误、系统容量超限或检测到重复安装冲突，则返回相应的底层 Error 错误码。
    ...
    virtual void closed(SessionGeneration generation, std::optional<Error::Code> error, Clock::time_point now) = 0;
    
    // 从控制循环周期性调用，用于驱动网络连接。
    // 用于取得至多一个已经到达网络重试退避期限或待发起的成员快照，
    // 取出时内部会同步登记处于 pending 等待中状态，以防止针对该成员造成过量的重复拨号。
    // 参数 now: 当前单调时间戳。
    // 返回: 如果当前没有可用目标则返回空；所有被由于本函数返回的成员快照，系统承诺都必须最终回馈调用 accept（成功接入）或 failed（拨号崩溃）以完结其挂起状态。
    ...
    virtual void failed(const Member& target, Error::Code error, Clock::time_point now) = 0;
    ...
    // 返回: 代表瞬间状态的 NetworkStatus 对象。
    virtual NetworkStatus status() const = 0;
};

// 策略工厂指针类型定义。
// 根据输入的 Config 参数返回具体的策略实例对象（如 StarPolicy 或 PlanetPolicy）。
using PolicyFactory = std::unique_ptr<Policy> (*)(const Config&);

// 共享的连接候选人替换逻辑规则检查。
// 规定: 来自同一次部署的连接实体不能自行改变身份角色（Role）或者底层监听端点; 而如果存在同一代次 epoch，则必须保证新旧成员的每个关键字段的值完全相等无歧义。
// candidate 和 current 两参数均须在调用该函数前完成预先格式校验;
// 参数 candidate: 期望覆盖当前状态的新成员对象。
// 参数 current: 被对比的目前正处在系统维护中的现存成员对象。
// 返回: 新成员更新代次更高更优则返回 true；两者完全一致无变化返回 false；
...
// 计算带有抖动的退避重试延迟。
// 失败等待时间的增长有固定上限（在 config 中定义），当达到最大退避边界时仍会保留随机抖动偏移以防大量重连拥堵，计算过程并不使用系统当前时间作为不安全的弱随机源。
// 参数 failures: 当前连续发生的失败次数记录，当其为 0 时也必须与首次失败采用相同的基础退避值起点;
// 参数 config: 包含关于延迟边界要求的配置，逻辑上需确保内部满足 0 < reconnect_min <= reconnect_max。
// 参数 salt: 一个用于使不同节点保持错开稳定的去同步时间扰动值，它的具体返回值一定分布处于配置的 reconnect_min..reconnect_max 这个区间内，且严禁直接充当任何系统的安全级别随机决策依赖。
// 返回: 最终计算得到的退避毫秒时间。
