...
public:
    ...
    virtual Result<std::vector<SessionGeneration>> accept(const Member& remote, Policy::Direction direction, SessionGeneration generation,
    ...
    virtual void closed(SessionGeneration generation, std::optional<Status::Code> error, Clock::time_point now) = 0;
    ...
    virtual void failed(const Member& target, Status::Code error, Clock::time_point now) = 0;
    ...
    virtual Policy::NetworkStatus status() const = 0;
};
