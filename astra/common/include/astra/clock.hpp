...
public:
    ...
    struct UnixEpoch;
    using Time = std::chrono::time_point<UnixEpoch, std::chrono::nanoseconds>;
    ...
    struct Reading {
        ...
        std::expected<Time, DeadlineError> deadline_after(std::chrono::nanoseconds ttl) const noexcept;
    };
    ...
    std::optional<EpochClock::Reading> now(ElapsedTime local) const;
    // 校验并外推至消费时刻, 替换残余偏差; 无效/过期/重复观测不改变原模型.
    bool publish(const EpochClock::Estimate& estimate);
    bool publish(const EpochClock::Estimate& estimate, ElapsedTime local);
...
private:
    ...
    std::optional<EpochClock::Reading> read(ElapsedTime local) const;
    bool observe(const EpochClock::Estimate& estimate, ElapsedTime local);
    // 默认入口在取得锁之后采样, 避免并发读取制造伪反序.
    mutable std::mutex mutex_;
    // 最大额外调速, 不代表底层振荡器准确度.
    const std::uint32_t slew_ppm_;
    // 空表示从未初始化公共纪元锚点.
    std::optional<EpochClock::Estimate> estimate_;
...
using ElapsedTime = EpochClock::ElapsedTime;
