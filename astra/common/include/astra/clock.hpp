// 功能: 将经过时间和可信观测合成为连续 Unix 时间, 数据只保存这一时间轴的绝对截止.
#pragma once
...
struct UnixEpoch;
using EpochTime = std::chrono::time_point<UnixEpoch, std::chrono::nanoseconds>;
...
struct ElapsedClock {
    ...
    using time_point = std::chrono::time_point<ElapsedClock>;
    static constexpr bool is_steady = true;
    // 内核读取失败或计数越界时抛异常, 不用零值伪装成功.
    static time_point now();
};
using ElapsedTime = ElapsedClock::time_point;
...
struct EpochReading {
    ...
    std::expected<EpochTime, DeadlineError> deadline_after(std::chrono::nanoseconds ttl) const noexcept;
};

// 进程级短锁模型, 读取不分配, 不写盘, 不调用 RPC.
class EpochClock {
public:
    // Star 默认 1000 ppm, 为跟踪 Pulsar 的 500 ppm 调速保留余量; 有效范围 1..1000.
    explicit EpochClock(std::uint32_t slew_ppm = 1000);
    // 未初始化/设施故障返回空; 失联只降低 ready, 不清除锚点.
    std::optional<EpochReading> now() const;
    // 测试/确定性驱动入口, local 必须顺序提交, 反序视为计时设施故障.
    std::optional<EpochReading> now(ElapsedTime local) const;
    // 校验并外推至消费时刻, 替换残余偏差; 无效/过期/重复观测不改变原模型.
    bool publish(const ClockEstimate& estimate);
    bool publish(const ClockEstimate& estimate, ElapsedTime local);
...
private:
    ...
    std::optional<EpochReading> read(ElapsedTime local) const;
    bool observe(const ClockEstimate& estimate, ElapsedTime local);
    // 默认入口在取得锁之后采样, 避免并发读取制造伪反序.
    mutable std::mutex mutex_;
    // 最大额外调速, 不代表底层振荡器准确度.
    const std::uint32_t slew_ppm_;
    // 空表示从未初始化公共纪元锚点.
    std::optional<ClockEstimate> estimate_;
    // 与 epoch_ 成对的本地经过时间锚点.
    mutable ElapsedTime local_{};
    // 已公开时间, observe 不跳跃替换.
    mutable EpochTime epoch_{};
