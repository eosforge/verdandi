...
def run(binaries: Path, polaris: Path, astrolabe: Path, soak=None):
    ...
    with tempfile.TemporaryDirectory(
        ...
        def wait_for(process, log, predicate, seconds=20):
            if soak is None:
                return ordinary(process, log, predicate, seconds=seconds)
            until = min(deadline, time.monotonic() + seconds)
            while time.monotonic() < until:
                soak.check(owned)
                if process.poll() is not None:
                    raise RuntimeError(
                        f"Owned process exited before expected event: {log.name}"
                    )
                for value in reversed(events(log)):
                    if predicate(value):
                        return value
                time.sleep(0.05)
            raise TimeoutError(
                f"Owned service did not reach expected state: {log.name}"
            )
        ...
        def finished(process, seconds):
            ...
            if process.poll() is None:
                raise TimeoutError("Owned native probe did not finish")
            return process.returncode

        def start(name, command):
            log = directory / f"{len(owned)}-{name}.log"
            # 子进程已经继承描述符, 父进程立即关闭自己的副本, 长测不累积历史日志句柄.
        ...
        def node(name, internal, external):
            process, log = start(
                ...
                [
                    ...
                    # 只有 A 被管理面采集. 不为 B/C 开随机指标监听, 否则可能抢到尚未绑定的固定业务端口.
                    *(
                        [f"--metrics=127.0.0.1:{metrics_port}"]
                        if internal == star_port
                        else []
                    ),
                    "--auth=true",
                    "--tls=true",
                    ...
                    f"--super={addresses['admission']}",
                    "--galaxy=alpha",
                    f"--status-interval-seconds={60 if soak else 1}",
                ],
            )
            endpoint = wait_for(
        ...
        if soak:
            soak.retire(
                reader
            )  # 此探针达到 version 4 后自行正常退出, 由下方检查退出码.
        ...
        nodes = [
            (star, star_port, comet_port, endpoint),
            (second, second_port, second_comet_port, second_endpoint),
            (third, third_port, third_comet_port, third_endpoint),
        ]
        limit = time.monotonic() + (soak.faults if soak else 0)
        cycle = 0
        while True:
            index = cycle % 3
            selected = nodes[index:] + nodes[:index]  # 轮流故障, 其余两个来源保持常驻.
            hold = (
                min(soak.interval, max(0, int(limit - time.monotonic()))) if soak else 0
            )
            mesh, mesh_log = start(
                "mesh",
                [
                    ...
                    *(item[3] for item in selected),
                    ...
                    str(hold),
                    "failure",
                    str(soak.records if soak else 1),
                    str(cycle),
                ],
            )
            if soak:
                soak.retire(mesh)  # 探针正常结束由阶段断言负责, 不是常驻服务.
            wait_for(
                mesh,
                mesh_log,
                lambda value: value.get("event") == "mesh_ready",
                seconds=hold + 75,
            )
            victim = selected[0][0]
            if soak:
                soak.retire(victim)
            victim.kill()  # 特意模拟突然丢失, 不先注销注册或正常关闭.
            if victim.wait(timeout=10) == 0:
            ...
            restored, _, address = node(
                f"star-{index}-restarted", selected[0][1], selected[0][2]
            )
            if address != selected[0][3]:
                raise RuntimeError("Restart did not preserve the deployment endpoint")
            nodes[index] = (restored, selected[0][1], selected[0][2], address)
            if finished(mesh, 100) != 0 or not any(
                event.get("event") == "mesh_recovered" for event in events(mesh_log)
            ):
                raise RuntimeError(
                    "Multi-Star native recovery or resource cleanup failed"
                )
            cycle += 1
            if soak:
                soak.record("recovered", cycle=cycle, star=index)
        ...
        for process in (*[item[0] for item in nodes], management, authority, pulse):
            stop(process)
        if soak is None:
            for _, log in owned:
                events(
                    log
                )  # 短测试解析完整日志; 长测在 check 中增量扫描, 不受短测的 8 MiB 上限干扰.
        if soak:
            soak.check(owned, force=True)
            soak.record("completed", cycles=cycle)
        print(
