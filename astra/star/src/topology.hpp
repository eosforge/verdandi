// 功能: 定义 Star 节点的成员拓扑索引以及全双工(双向)的会话管理策略。
...
namespace astra {
// StarTopology 类继承自 Policy 接口。作为 Star 节点的核心网络拓扑策略。
...
public:
    ...
    // 返回值: 
    // - 成功返回空。
    // - 如果超出了容量限制返回 Error::Code::capacity 错误。
    // - 如果名单内容（如顺序/去重）存在问题，返回 Error::Code::identity。
    // - 如果尝试多次初始化，返回 Error::Code::conflict。任何失败都会保留原有的旧状态不变。
    ...
    // 返回值: 成功时返回包含了需要被取消的过期 SessionGeneration 列表（如果是平滑替换）；
...
private:
    ...
    struct Entry {
        ...
        // 这一布局必须与 Direction 枚举的值及其顺序保持严格一致。
        ...
        Clock::time_point next{};            // 默认值为纪元 0。下一次允许向其发起出站重试的到期时间。
        Clock::time_point connected{};       // 默认值为纪元 0。当前出站连接成功建立的时间，用于判定连接稳定性。
    };
...
// 工厂函数: 根据校验无误的 config 创建并返回一个尚未初始化的 StarTopology 策略实例。
