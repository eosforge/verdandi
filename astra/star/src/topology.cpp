...
Result<std::vector<Generation>> Star::accept(const Member& remote, Policy::Direction direction, Generation generation, const std::optional<Member>& expected) {
    ...
    if (!local_ || remote.galaxy != local_->galaxy || remote.id == local_->id || remote.principal == local_->principal || remote.address == local_->address || (remote.role == Member::Role::planet && direction != Policy::Direction::inbound)) {
    ...
    if (direction == Policy::Direction::outbound) {
        ...
        entry.connected = Steady::now(); // 更新连通时间.
    }
