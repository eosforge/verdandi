...
Result<std::vector<SessionGeneration>> StarTopology::accept(const Member& remote, Policy::Direction direction, SessionGeneration generation,
    ...
    if (!local_ || remote.galaxy != local_->galaxy || remote.id == local_->id || remote.principal == local_->principal || remote.address == local_->address ||
        (remote.role == Member::Role::planet && direction != Policy::Direction::inbound)) {
    ...
    if (slot) {
        ...
        return Status::conflict("Star direction is already active");
    }
...
void StarTopology::closed(SessionGeneration generation, std::optional<Status::Code>, Clock::time_point now) {
    std::lock_guard lock(mutex_);
    for (auto& [principal, entry] : members_) {
        for (std::size_t index = 0; index < entry.sessions.size(); ++index) {
            ...
            if (entry.sessions[index] == generation) {
                ...
                if (index == static_cast<std::size_t>(Policy::Direction::outbound)) {
                    // 判断断开前该连接是否达到了稳定期标准。
...
std::optional<Member> StarTopology::due(Clock::time_point now) {
    ...
    if (local_) {
        for (auto& [principal, entry] : members_) {
            ...
            if (entry.member.role == Member::Role::star && !entry.pending && !entry.sessions[0] && now >= entry.next) {
...
void StarTopology::failed(const Member& target, Status::Code, Clock::time_point now) {
    std::lock_guard lock(mutex_);
...
Policy::NetworkStatus StarTopology::status() const {
    std::lock_guard lock(mutex_);
