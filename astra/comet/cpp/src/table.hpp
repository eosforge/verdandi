...
public:
    ...
    std::size_t footprint() const noexcept {
        auto total = bytes_ + sizeof(Table) + size_ * (sizeof(typename Page::value_type) + Record::overhead + 32);
        for (const auto& page : pages_) {
            if (page) {
                total += sizeof(Page) + 32 + page->bucket_count() * sizeof(void*);
            }
        }
        return total;
    }
    ...
    public:
        ...
        explicit Draft(const Table& base) : pages_(base.pages_), size_(base.size_), bytes_(base.bytes_) {}
        ...
        std::size_t footprint() const noexcept {
            auto total = bytes_ + sizeof(Draft) + size_ * (sizeof(typename Page::value_type) + Record::overhead + 32);
            for (std::size_t slot = 0; slot < pages_.size(); ++slot) {
                const auto* page = dirty_[slot] ? dirty_[slot].get() : pages_[slot].get();
                if (page) {
                    total += sizeof(Page) + 32 + page->bucket_count() * sizeof(void*);
                }
            }
        ...
        // 完整替换一个 Key, 载荷由 Record 自己共享/拥有; 本页第一次变化才复制其他条目.
        void set(std::string key, Record record) {
            ...
            const auto cost = key.size() + record.bytes();
            if (cost > SIZE_MAX - (bytes_ - previous)) {
                throw std::length_error("View byte count overflow");
            }
            const bool added = old == page.end();
            page.insert_or_assign(std::move(key), std::move(record));
            bytes_ = bytes_ - previous + cost;
            size_ += static_cast<std::size_t>(added);
        }

        // 缺失 Delete 无变化, 不复制空页; 删除完成后空页回收, 不保留历史桶水线.
        void erase(std::string_view key) {
            const auto slot = Hash{}(key) & 255;
            const auto* current = dirty_[slot] ? dirty_[slot].get() : pages_[slot].get();
            if (!current || !current->contains(key)) {
                return;
            }
            auto& page = edit(slot);
        ...
        Table finish() && {
            ...
            for (std::size_t slot = 0; slot < dirty_.size(); ++slot) {
                if (dirty_[slot]) {
                    result.pages_[slot] = dirty_[slot]->empty() ? nullptr : std::move(dirty_[slot]);
                }
            }
            result.size_ = size_;
    ...
    private:
        ...
        Page& edit(std::size_t slot) {
            if (!dirty_[slot]) {
                dirty_[slot] = pages_[slot] ? std::make_shared<Page>(*pages_[slot]) : std::make_shared<Page>();
            }
            return *dirty_[slot];
        }
