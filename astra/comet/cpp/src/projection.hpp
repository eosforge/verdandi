...
struct Contents {
    Table<Datum> data;       // 不可变分页字典, 跨快照共享未变化页.
    std::string instance;    // 对应这份完整数据的实际 Star ID.
    std::uint64_t version{}; // Almanac 权威版本, 零允许合法空底稿.
};

// Almanac 下行安装策略. 单个网络任务顺序调用, 当前根只在 complete 全部校验通过后替换.
class Projection {
public:
    using API = Reader;                                // 本投影的公共读取接口.
    using Reply = proto::comet::v1::AlmanacWatchReply; // 私有具体下行消息.
    using Service = proto::comet::v1::Almanac;         // 私有生成 Watch 服务.
    ...
    Projection(Scope scope, std::string target, std::size_t bytes, std::size_t records, Reserve reserve = {});
    ~Projection(); // 归还网络受控计费, 应用额外保有旧 View 的实际寿命不受影响.
    ...
    Result<std::optional<Reader::View>> accept(const proto::comet::v1::AlmanacWatchReply& page);
    // 状态只包装最近完整内容, 不读写网络, 不偷偷改变已经给应用的 View.
    Reader::View view(Reader::State state, std::optional<Error> error = {}) const;
    // 放弃未完成页面, 不取消网络或改变旧视图; 分配失败/断流时同样调用.
    void discard() noexcept;

private:
    // 统一失败路径使本流失效, 不把损坏页面当作可忽略的心跳.
    Result<std::optional<Reader::View>> fail(Error::Code code);
    ...
    const std::size_t records_;                // 可安装的最多记录数, 初值来自 Reader::Options.
    Reserve reserve_;                          // 只作共享预算调整, 不回调用户或取得 Reading 的锁.
    std::shared_ptr<const Contents> current_;  // 最后完整发布, 无网络寿命依赖.
    std::optional<Table<Datum>::Draft> draft_; // 当前批次的私有写页, 未完成不对外读取.
