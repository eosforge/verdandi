...
// 持续读取一个 Ephemeris 范围, 私有网络状态与应用拥有的不可变 View 分离.
class Observer {
public:
    // 对外始终是完整注册, Attr/Data 分别不可变共享, Data 更新不复制 Attr.
    struct Record {
        Value attr; // 注册时固定的属性正文, 合法零字节仍为非空指针.
        Value data; // 最近完整安装的动态正文, 不含 TTL/来源组/内部续租序号.
    };
    // 每次 load/回调取得时的本地状态, 不会反向修改应用已持有的旧 View.
    enum class State {
        waiting, // 尚未安装任何完整注册基线.
    ...
    // 轻量拥有式视图, 查询不依赖 Observer/Client 仍然存活, 不返回可写容器或生成协议类型.
    class View {
    public:
        ...
        const std::string& target() const noexcept;              // 空为整个 Scope, 非空为精确 UUID.
    ...
    private:
        ...
        std::shared_ptr<const detail::Registrations> contents_;
    ...
    ~Observer();                                        // 非阻塞 close, 不在析构等待网络或 join 线程.
    View select() const;                                // 最近完整内容与当前状态, 不主动发请求.
