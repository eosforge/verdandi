...
class Watching;
using Subscribing = Watching<Subscription>;
struct Publications;
} // namespace detail
class Client;

// 持续读取一个 Catalog 范围, 私有网络状态与应用拥有的不可变 View 分离.
class Subscriber {
public:
    // 完整动态记录, 业务内容版本与 View::version 的本地恢复游标独立.
    struct Record {
        std::uint64_t version{}; // 来自唯一业务发布者的正版本.
        Value value;             // 完整不可变正文, 非空所有者允许零字节.
    };

    // 每次 watch/回调取得时的本地状态, 不会反向修改应用已持有的旧 View.
    enum class State {
        waiting, // 尚未安装任何完整动态基线.
    ...
    // 轻量拥有式视图, 查询不依赖 Subscriber/Client 仍然存活, 不返回可写容器或生成协议类型.
    class View {
    public:
    ...
    private:
        friend class detail::Subscription;
        template <class Policy>
        friend class detail::Watching;
        // 非拥有类型擦除桥接, context 和 visitor 仅在本次同步调用有效.
        void visit(void* context, void (*visitor)(void*, std::string_view, const Record&)) const;
        // 完整内容只读共享, 不持有网络核心、RPC 上下文或应用回调.
        std::shared_ptr<const detail::Publications> contents_;
    ...
    ~Subscriber();                                      // 非阻塞 close, 不在析构等待网络或 join 线程.
    View watch() const;                                 // 最近完整内容与当前状态, 不主动发请求.
