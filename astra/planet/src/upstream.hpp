...
public:
    ...
    Result<std::vector<Generation>> accept(const Member& remote, Policy::Direction direction, Generation generation,
    ...
    // 被隔离的候选节点依然会被计入候选总数中。
    // 返回值: 包含初始化状态、候选数量、活动节点等信息的 Policy::State 结构体。
...
private:
    ...
    struct Candidate {
        ...
        Steady::time_point next{};      // 默认值为纪元 0。下一次允许发起拨号连接的最早时间点（受退避延迟影响）。
        Steady::time_point connected{}; // 默认值为纪元 0。记录该候选节点成功建立连接的时间点，用于判断连接是否稳定。
    };
    ...
    void record_failure(Candidate& candidate, Status::Code error, Steady::time_point now);
    ...
    std::optional<std::pair<Principal, Generation>> active_; // 记录当前成功建立且处于活动状态的上游。保存目标的主体标识 (Principal) 以及对应的会话代次。
    bool refresh_{};                                                // 默认 false。一个标志位，用于标识是否触发了强制刷新请求。
    Steady::time_point next_refresh_{};                              // 记录下一次允许执行名单刷新的最早时间点。
};
