...
Result<void> PlanetUpstream::initialize(const Member& local, std::span<const Member> members) {
    ...
    if (members.size() > candidates_.capacity()) {
        return Error::capacity("Too many Planet candidates");
    }
    ...
    for (const auto& member : members) {
        ...
        if (!member.validate() || member.role != Role::star || member.galaxy != config_.galaxy || member.principal == local.principal ||
            ...
            return Error::identity("Invalid Planet candidate list");
        }
        previous = order;
        // 验证通过，放入暂存向量。
    ...
    if (local_ && *local_ != local) {
        return Error::identity("Refresh changed local identity");
    }
...
Result<std::vector<SessionGeneration>> PlanetUpstream::accept(const Member& remote, Direction direction, SessionGeneration generation,
    ...
    if (!local_ || active_ || direction != Direction::outbound || !expected || remote.role != Role::star || remote.galaxy != local_->galaxy) {
        return Error::identity("Planet requires one authorized outbound Star");
    }
    ...
    // 未找到匹配的合法候选上游。
    return Error::identity("Candidate no longer authorized");
}

// record_failure 实现: 专门用于处理和记录失败的私有函数。
void PlanetUpstream::record_failure(Candidate& candidate, Error::Code error, Clock::time_point now) {
    // 结束在途状态，并标记本轮已尝试过。
    ...
    // 除非后续收到合法新成员代次替换了该候选。
    candidate.quarantined = candidate.quarantined || error == Error::Code::identity || error == Error::Code::protocol || error == Error::Code::conflict;
...
// closed 实现: 响应连接断开事件。
void PlanetUpstream::closed(SessionGeneration generation, std::optional<Error::Code> error, Clock::time_point now) {
    std::lock_guard lock(mutex_);
    ...
    for (auto& candidate : candidates_) {
        if (candidate.member.principal == active_->first) {
            ...
            record_failure(candidate, error.value_or(Error::Code::transport), now);
            break;
        }
    }
    // 连接已关闭，清空 active_ 状态。
...
std::optional<Member> PlanetUpstream::due(Clock::time_point now) {
    ...
    // 第二优先级：节点所属组与本地配置组的不同程度（尽量选择同组节点，如果配置偏好同组的话，这里 c.member.group != config_.group 表示不同组的权重较高/被排在后面）。
    // 第三优先级：通过 candidate_rank 计算出来的进程特定哈希值，实现负载均衡。
    // 第四优先级：作为保底，使用节点的 id 排序。
...
void PlanetUpstream::failed(const Member& target, Error::Code error, Clock::time_point now) {
    std::lock_guard lock(mutex_);
    for (auto& candidate : candidates_) {
