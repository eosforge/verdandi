...
测试分为原生规则/分配故障、真实 SQLite 持久、真实 gRPC/TLS、SDK 生命周期及跨进程故障恢复; 各层不能互相替代. cpp_comet_process 包含专有控制服务/三台 Star、Polaris 重启、旧缓存读取、指标抓取、源 Star 强制退出、SDK 换节点/新 UUID 和原端口重启; 所有进程、数据库和秘密材料由夹具清理. 不启动 Planet/Moon, 不依赖 Comet Go.
...
- **R04** Star 生成原始 16 字节 UUIDv4, 校验长度及版本/变体位, 含 NUL 的合法值原样往返, 拒绝 36 字符文本别名. 一条原生 Attr/Data 记录、一个租约和本机来源组位置原子建立. 随机源失败、冲突或非法格式不能覆盖旧记录, 无指定 UUID/内容去重, Session/节点 ID 格式保持独立.
...
- **S21** 同 Client 的多个 Publisher/Subscriber 中仅一个连续更新, 其他对象仍按各自期限自动保活; 静音续期不推进公开版本. 共享 close 排空所有对象, 无外部流量也能完成退出.
...
标准系统模型为至少三台 Star, 三台同时承担各自来源的写入, 订阅覆盖其他节点的来源. 单/双节点及无网络组件用例用于定向诊断, 不能代替此验收. 同时保留单 Scope 多来源合并、多 Scope 独立写入、一台恢复时另外两台持续写入/续租三类负载; 一台慢对端不能停止其他链路. 来源 ACK、最终内容、TTL、无副本反向广播与资源回收均须检查. 三节点基线接线见 [统一应用基线](baseline/README.md), 新模型是否已执行以验证页为准.
