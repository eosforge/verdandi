...
from testkit.services import Host, ServiceProcess
...
class ServiceHarnessTests(unittest.TestCase):
    def test_sanitizer_failure_is_rejected_even_during_forced_cleanup(self):
        ...
        process = ServiceProcess(
            [sys.executable, "-B", "-c", "import sys; print('WARNING: ThreadSanitizer: data race', flush=True); sys.exit(66)"], directory / "sanitizer.log"
        )
    ...
    def test_graceful_stop_rejects_already_failed_process(self):
        host = Host()
        process = ServiceProcess([sys.executable, "-B", "-c", "raise SystemExit(66)"], host.directory / "early-exit.log")
    ...
    def test_bad_login_uses_owned_copy_and_keeps_process_name(self):
        original = ROOT / "peer/tests/fixtures/peer-a/login.json"
        ...
        try:
            with patch("testkit.services.ServiceProcess") as process:
                ...
                host.call(
                    "start", name="bad-login", kind="peer", role="peer-a", login_case="wrong-password", address="127.0.0.1:7443", supervisor="127.0.0.1:7442"
                )
                ...
                self.assertEqual(json.loads((copied / "login.json").read_text())["password"], "wrong-public-test-password")
        ...
        try:
            with patch("testkit.services.ServiceProcess") as process:
                with self.assertRaises(ValueError):
                    host.call("start", name="bad", kind="peer", role="peer-a", login_case="arbitrary")
    ...
    def test_cli_nonzero_success_claim_is_rejected(self):
        host = Host()
        try:
            with patch("testkit.services.subprocess.run", return_value=SimpleNamespace(returncode=1, stdout="version", stderr="")):
    ...
    def test_cli_old_identity_instructions_are_rejected(self):
        ...
        try:
            outputs = [
                SimpleNamespace(returncode=0, stdout="peer 0.1.0", stderr=""),
                SimpleNamespace(returncode=0, stdout="UUID and process key are generated", stderr=""),
            ]
