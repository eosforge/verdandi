...
func candidate(index int) Member {
	return Member{
		...
		PeerID:    fmt.Sprintf("%08x000040008000000000000000", index),
...
func TestRolesHaveIndependentCapacityAndRetryMetadataIsImmutable(t *testing.T) {
	store, err := Open(filepath.Join(t.TempDir(), "members.db"), 1)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = store.Close() })
	if _, err := store.Register("alpha", candidate(1), 0); err != nil {
		t.Fatal(err)
	}
	planet := candidate(2)
	planet.Role = Planet
	if _, err := store.Register("alpha", planet, 0); err != nil {
	...
	for _, changed := range []Member{candidate(2), func() Member { changed := planet; changed.Group = "other"; return changed }()} {
		if _, err := store.Register("alpha", changed, 0); !errors.Is(err, ErrConflict) {
			t.Fatal("retry changed role/group:", err)
		}
	}
	if _, err := store.Register("alpha", candidate(3), 0); !errors.Is(err, ErrCapacity) {
		t.Fatal("Star capacity ignored")
	}
	other := candidate(4)
	other.Role = Planet
	if _, err := store.Register("alpha", other, 0); !errors.Is(err, ErrCapacity) {
		t.Fatal("Planet capacity ignored")
	}
}

func TestConcurrentRegistrationReturnsOrderedCompleteSnapshots(t *testing.T) {
	store, err := Open(filepath.Join(t.TempDir(), "members.db"), 32)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = store.Close() })
	...
	for index := range count {
		workers.Go(func() {
			snapshot, err := store.Register("alpha", candidate(index+1), 0)
			results <- snapshot
			failures <- err
		})
	}
	...
	for snapshot := range results {
		...
		for index := 1; index < len(snapshot); index++ {
			if snapshot[index-1].PeerID >= snapshot[index].PeerID {
...
func TestRetryPersistenceAndRestartFencing(t *testing.T) {
	...
	first := candidate(1)
	for range 3 {
		snapshot, err := store.Register("alpha", first, 0)
	...
	if epoch, err := store.Epoch("alpha", first.Principal); err != nil || epoch != 1 {
	...
	if _, err := store.Register("alpha", first, 0); !errors.Is(err, ErrConflict) {
		t.Fatalf("stale retry replaced new process: %v", err)
	}
	if _, err := store.Register("alpha", candidate(3), 0); !errors.Is(err, ErrCapacity) {
		t.Fatalf("capacity not enforced: %v", err)
	}
	if _, err := store.Register("beta", candidate(3), 0); err != nil {
		t.Fatalf("cluster capacity leaked: %v", err)
	}
}

func TestConflictsDoNotMutateMembers(t *testing.T) {
	store, err := Open(filepath.Join(t.TempDir(), "members.db"), 8)
	if err != nil {
		t.Fatal(err)
	}
	t.Cleanup(func() { _ = store.Close() })
	first := candidate(1)
	if _, err := store.Register("alpha", first, 0); err != nil {
		t.Fatal(err)
	}
	tests := []struct {
		name   string
		...
		{"other-principal-same-address", func(m *Member) { m.Principal = candidate(2).Principal; m.PeerID = candidate(2).PeerID }},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			original, err := store.Register("alpha", first, 0)
...
func TestCorruptMemberFailsClosedAndReleasesLock(t *testing.T) {
	...
	if err := store.db.Update(func(tx *bolt.Tx) error {
		...
		return bucket.Put([]byte(candidate(1).Principal), []byte(`{"peer_id":"broken"}`))
	}); err != nil {
		t.Fatal(err)
	}
	if err := store.Close(); err != nil {
	...
	if reopened, err := Open(path, 8); !errors.Is(err, ErrInvalid) {
		if reopened != nil {
			_ = reopened.Close()
		}
...
func TestValidationAndReturnedMemoryOwnership(t *testing.T) {
	...
	for _, cluster := range []string{"", "a/b", "中文", string(bytes.Repeat([]byte{'a'}, 65))} {
		if _, err := store.Register(cluster, candidate(1), 0); !errors.Is(err, ErrInvalid) {
	...
	for _, address := range []string{
		...
		if _, err := store.Register("alpha", member, 0); !errors.Is(err, ErrInvalid) {
			t.Fatalf("invalid address accepted: %v", err)
		}
	}
	snapshot, err := store.Register("alpha", candidate(1), 0)
	if err != nil {
		t.Fatal(err)
	}
	snapshot[0].PeerID = "changed"
	snapshot[0].Group = "changed"
	again, err := store.Register("alpha", candidate(1), 0)
	if err != nil || again[0].PeerID != candidate(1).PeerID || again[0].Group != "default" {
