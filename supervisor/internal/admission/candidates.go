...
func encodeMember(cluster string, member membership.Member) *wire.Member {
...
func candidates(stars []*wire.Member, group, process string, round uint32) []*wire.Member {
	local, remote := make([]*wire.Member, 0), make([]*wire.Member, 0)
	...
	rotate := func(pool []*wire.Member) []*wire.Member {
		if len(pool) == 0 {
			return pool
		}
	...
	result := make([]*wire.Member, 0, l+r)
	result = append(result, local[:l]...)
	result = append(result, remote[:r]...)
	slices.SortFunc(result, func(a, b *wire.Member) int {
