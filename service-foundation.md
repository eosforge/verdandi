...
[身份与准入契约](cluster/identity-contract.md)定义当前命名、协议 v1 和升级边界.

## 组织与依赖方向

| 层 | C++ Star/Planet | Go Supervisor | 责任 |
| --- | --- | --- | --- |
| 入口 | `astra/star/src/main.cpp`, `planet/src/main.cpp` | `supervisor/cmd/supervisor/` | 退出码、版本和进程组合 |
...
C++ 共用代码位于 `astra`, 自身标识为不透明 `Id = std::string`.
...
协议 v1 不兼容旧 Rust/v4, 旧 `peer_id` 成员库不会自动改写或清空.
开发验证使用独立数据库. Catalog/Registry 复制、业务恢复和 SDK 接入仍属于后续工作.

## 代码与依赖

遵循[编码规范](coding.md)和 [C++ 维护指南](astra/CONTRIBUTING.md).
...
C++ 固定依赖见[版本锁](astra/dependencies.lock.json)及[来源说明](cluster/cpp26-dependencies.md).
...
python3 astra/build.py regression --profile debug
```
