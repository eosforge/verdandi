//! 显式生成 Rust/Go 协议源码和稳定 MessageID, 不下载工具或依赖.
//!
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // bytes 字段统一使用 Bytes. Rust 按 protobuf package 合并输出, Go 按输入文件输出.
    let mut config = prost_build::Config::new();
    config
        .protoc_executable(&protoc)
        .bytes(["."])
        .out_dir(&scratch.0)
        .file_descriptor_set_path(scratch.0.join("descriptor.pb"));
    // 同一次显式生成同时输出消息和 Rust RPC 接口. Go 仍只需要 Supervisor 的消息类型.
    tonic_prost_build::configure()
        .codec_path("crate::codec::LimitedCodec")
        .out_dir(&scratch.0)
        .compile_with_config(config, &schemas, std::slice::from_ref(&proto_root))?;
    ...
    rustfmt.arg("--edition=2024").arg("--config-path").arg(root.join("peer/rustfmt.toml"));
    ...
    for entry in fs::read_dir(&scratch.0)? {
        ...
        let destination = match path.extension().and_then(|extension| extension.to_str()) {
            Some("rs") => root.join("peer/common/src/generated"),
    ...
    for directory in [root.join("peer/common/src/generated"), root.join("supervisor/internal/generated")] {
        if !directory.exists() {
            continue;
        }
...
fn generate_ids(output: &Path, proto_root: &Path) -> Result<String, Box<dyn Error>> {
    ...
    let mut registry = String::from("# Automatically assigned MessageIDs. Commit this file with peer.proto. Never reuse an ID.\n");
    for (id, name) in &ids {
    ...
    for (id, name) in &active {
        rust.push_str(&format!("{name} = {id},\n"));
        go.push_str(&format!("ID{name} MessageID = {id}\n"));
    }
    rust.push_str("}\nimpl TryFrom<u16> for MessageId { type Error = (); fn try_from(id: u16) -> Result<Self, Self::Error> { match id {\n");
    go.push_str(")\nfunc NewMessage(id MessageID) proto.Message { switch id {\n");
    for (id, name) in &active {
        rust.push_str(&format!("{id} => Ok(Self::{name}),\n"));
        go.push_str(&format!("case ID{name}: return &{name}{{}}\n"));
    }
    rust.push_str("_ => Err(()), } } }\npub trait WireMessage: ::prost::Message + Default { const ID: MessageId; }\n");
    go.push_str("default: return nil } }\nfunc IDOf(message proto.Message) (MessageID, bool) { switch message.(type) {\n");
    for (_, name) in &active {
        rust.push_str(&format!("impl WireMessage for {name} {{ const ID: MessageId = MessageId::{name}; }}\n"));
        go.push_str(&format!("case *{name}: return ID{name}, true\n"));
    }
    go.push_str("default: return 0, false } }\n");
    fs::write(output.join("message_ids.rs"), rust)?;
