...
export function validateGalaxyData(data: GalaxyData): void {
  ...
  for (const peer of data.peers) {
    if (!peer.id || peers.has(peer.id)) throw new Error(`Duplicate or empty Peer id: ${peer.id}`);
    peers.add(peer.id);
    validatePosition(peer.position);
    if (!/^#[\da-f]{6}$/i.test(peer.color)) throw new Error(`Invalid Peer color: ${peer.id}`);
    if (peer.status !== "available" && peer.status !== "unavailable") throw new Error(`Invalid Peer status: ${peer.id}`);
    for (const planet of peer.planets) {
      ...
      if (planet.peerId !== peer.id || !planetKinds.includes(planet.kind)) throw new Error(`Invalid planet owner or kind: ${planet.id}`);
  ...
  for (const link of data.links) {
    if (!peers.has(link.source) || !peers.has(link.target) || link.source === link.target) throw new Error("Invalid Peer link endpoints");
    if (edges.get(link.source)?.has(link.target)) throw new Error("Duplicate undirected Peer link");
