...
def main():
    ...
    templates = {
        name: x509.load_pem_x509_certificate((root / name / "cert.pem").read_bytes())
        for name in names
    }
    authority = ec.generate_private_key(ec.SECP256R1())
    subject = x509.Name(
        [x509.NameAttribute(NameOID.COMMON_NAME, "Verdandi PUBLIC TEST ECDSA CA")]
    )
    ca = (
        ...
        .add_extension(
            x509.SubjectKeyIdentifier.from_public_key(authority.public_key()),
            critical=False,
        )
        .add_extension(
    ...
    for name, previous in templates.items():
        ...
        # SKI/AKI 随本轮密钥重算, 满足 Python 严格 X.509 校验, 不复用旧密钥的标识.
        ...
        signer = (
            ec.generate_private_key(ec.SECP256R1()) if name == "rogue" else authority
        )
        certificate = builder.sign(signer, hashes.SHA256())
        (root / name / "ca.pem").write_bytes(
            ca.public_bytes(serialization.Encoding.PEM)
        )
        (root / name / "cert.pem").write_bytes(
