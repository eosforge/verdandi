...
`cluster/` 是一个 Cargo workspace, 共用锁文件和项目内构建目录:
...
目标更新 (2026-09-12): [Supervisor 签发不透明 Id](cpp26-skeleton-design.md#50-supervisor-签发不透明-starid-的目标规则)
由维护者确认. Star/Planet 不再依赖 UUIDv4、固定长度或十六进制格式, 只在准入边界验证受信签发凭证.
该迁移尚未实施, 本文下方的客户端 UUID 生成和校验描述保留为当前 v4 行为记录.
...
`--max-members=N` 在 Supervisor 和 Star 中分别限制 N 个 Star 和 N 个 Planet.
...
节点间使用 `StarTransport.OpenSession` 双向 gRPC 流, 每个方向传输 `SessionPacket` oneof.
...
Linux 默认 C++: `bash scripts/test-services.sh`; Rust 对照加 `--implementation=rust`.
Windows Rust 对照: `scripts/test-services.ps1 -Implementation rust`; C++26 服务要求 Linux.
