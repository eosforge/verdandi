...
use verdandi_planet::Planet as Peer;

/// 帮助/正常退出为 0, 参数错误为 2, 启动或运行失败为 1. 不依赖 stdin 的状态.
pub(crate) fn main() -> ExitCode {
    verdandi_peer_common::app::launch("planet", env!("CARGO_PKG_VERSION"), run)
}
...
async fn run(options: cli::Options, output: &mut impl Write) -> io::Result<()> {
        ...
        result = loop {
            ...
            tokio::select! {
                ...
                _ = status_clock.tick(), if options.status_seconds != 0 => {
                    ...
                    if let Err(error) = logging::write(output, "planet", "INFO", "status", json!({"peer_id": peer.peer_id(), "initialized": status.initialized,
                        "candidates": status.candidates, "upstream": status.upstream.as_ref().map(|session| json!({"peer_id": session.remote.peer_id,
                            "group": session.remote.group, "generation": session.generation, "address": session.remote.advertise}))})) { break Err(error); }
                }
                event = events.recv() => {
                    let logged = match event {
                        Ok(event) => logging::event(output, "planet", event),
                        Err(RecvError::Lagged(count)) => logging::write(output, "planet", "WARN", "diagnostic_lagged", json!({"count": count})),
    ...
    let stopping = logging::write(output, "planet", "INFO", "stopping", json!({}));
    ...
    logging::write(output, "planet", "INFO", "stopped", json!({}))
}
